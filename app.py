import os
import subprocess
import json
from pathlib import Path

def is_git_repo(directory):
    """Check if the given directory is a Git repository."""
    try:
        subprocess.run(["git", "-C", directory, "status"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=True)
        return True
    except subprocess.CalledProcessError:
        return False

def get_git_info(directory):
    """Retrieve Git information for the repository."""
    try:
        # Get branch name
        branch = subprocess.check_output(
            ["git", "-C", directory, "rev-parse", "--abbrev-ref", "HEAD"],
            text=True
        ).strip()

        # Get last commit message
        last_commit = subprocess.check_output(
            ["git", "-C", directory, "log", "-1", "--pretty=%B"],
            text=True
        ).strip()

        # Check if working directory is clean
        status_output = subprocess.check_output(
            ["git", "-C", directory, "status", "--porcelain"],
            text=True
        )
        clean = not bool(status_output.strip())

        return {
            "branch": branch,
            "last_commit": last_commit,
            "clean": clean
        }
    except subprocess.CalledProcessError:
        return None

def load_config(config_path):
    """Load the configuration file."""
    with open(config_path, "r") as f:
        return json.load(f)

def main(config_path):
    """Main function to process directories and print Git information."""
    config = load_config(config_path)
    directories = config.get("directories", [])

    results = []
    for directory in directories:
        dir_path = Path(directory).resolve()
        if not dir_path.is_dir():
            results.append({"directory": str(dir_path), "error": "Not a valid directory"})
            continue

        if not is_git_repo(dir_path):
            results.append({"directory": str(dir_path), "error": "Not a Git repository"})
            continue

        git_info = get_git_info(dir_path)
        if git_info:
            results.append({
                "directory": str(dir_path),
                "branch": git_info["branch"],
                "last_commit": git_info["last_commit"],
                "clean": git_info["clean"]
            })
        else:
            results.append({"directory": str(dir_path), "error": "Failed to retrieve Git info"})

    # Print results
    print(f"{'Directory':<40} {'Branch':<20} {'Clean':<10} {'Last Commit':<40} {'Error':<20}")
    print("=" * 130)
    for result in results:
        print(f"{result.get('directory', ''):<40} "
              f"{result.get('branch', ''):<20} "
              f"{str(result.get('clean', '')):<10} "
              f"{result.get('last_commit', ''):<40} "
              f"{result.get('error', ''):<20}")

if __name__ == "__main__":
    # Example usage: python script.py config.json
    import sys
    if len(sys.argv) != 2:
        print("Usage: python app.py <config.json>")
        exit(1)
    main(sys.argv[1])
